...
const routers = function (obj, activeTab, menu, value) {
  ...
  if (value === null) {
    switch (menu) {
      case 'QY病历':
        url = `wt4_clipalm?plat=client&drg=QY&page=${obj.$store.state.Edit.wt4Page}&version=${version}`
        router = 'drgwork_wt4/expert'
        break
      case '未入组病历':
        url = `wt4_clipalm?plat=client&drg=0000&page=${obj.$store.state.Edit.wt4Page}&version=${version}`
        router = 'drgwork_wt4/expert'
        break
      case '低风险死亡病历':
        url = `wt4_clipalm?plat=client&sf0108=5&page=${obj.$store.state.Edit.wt4Page}&version=${version}`
        router = 'drgwork_wt4/expert'
        break
      case '费用异常病历':
        url = `wt4_clipalm?plat=client&cv=1&page=${obj.$store.state.Edit.wt4Page}&version=${version}`
        router = 'drgwork_wt4/expert'
        break
      case '填报异常病历':
